...
class Balance:
    ...
    def Balance(self, filename):
        ...
        if (self.CargoGrid.Balance_Check() == False):
            ...
            # if ship container is unbalanced, make a list of all containers on the heavier side
            self.CargoList()
            while (balanced == False):
                ...
                self.CargoGrid.Grid_Copy(  # set cargo grid to grid wth largest weight ratio
                    self.nodeList[0])
                output += "Move cargo " + \
                    "from ( " + str(self.CargoGrid.old_pos[0]) + "," + str(self.CargoGrid.old_pos[1]) + \
                    ")  to (" + str(self.CargoGrid.new_pos[0]) + \
                    "," + \
                    str(self.CargoGrid.new_pos[1]) + ")" + "\n"
                self.ProgressionList.append(
                    copy.deepcopy(self.CargoGrid))
                # check if new cargo grid is balanced
                if (self.CargoGrid.Balance_Check() == True):
                    # with open(file, "w") as file:  # output operations list
                    # file.write(output)
                    balanced = True
